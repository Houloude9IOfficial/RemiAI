...
- **Web Push notifications** for scheduled tasks, background agents, and explicit `send_notification` calls
- Push notifications continue to work when the hosted web app is closed, and clicking one opens the originating chat
...
#### Web Push setup for self-hosted deployments

Web Push requires HTTPS on the public RemiAI URL (localhost is allowed for local browser testing). Generate one VAPID key pair once with `npx web-push generate-vapid-keys`, then configure the VPS/container with:
...
Open **Settings → Profile → Notifications** on each browser or installed phone PWA and enable notifications. The server stores subscriptions in the persistent RemiAI database and automatically removes expired browser subscriptions. Keep the VAPID keys stable across deployments; changing them invalidates existing subscriptions and requires users to enable notifications again.

A background researcher can continue running on the VPS after the phone/browser is closed when started through the durable scheduler/automation path. Completion notifications include a short result preview and link back to the relevant chat. The Electron app uses native OS notifications from its own local RemiAI server while its process is running, including when its window is hidden to the tray. A local Electron process cannot receive an event generated by a separate VPS instance; hosted notifications use the browser/PWA Web Push subscription instead. A fully quit Electron process cannot receive local IPC notifications.
