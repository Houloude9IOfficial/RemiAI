/**
 * Helpers that derive the user's country/language from the browser-provided
 * locale (e.g. "en-US" → country US, language en) and, as a fallback, from
 * their IANA timezone (e.g. "Europe/Bucharest" → country RO). Used to make
 * web search results (Brave, NewsAPI) relevant to the user's region.
 */
...
export interface UserContext {
  ...
  country?: string;
  /** ISO 639-1 language code, e.g. "ro" */
  language?: string;
}

/**
 * Map of IANA timezone → ISO 3166-1 alpha-2 country code.
 * Based on the tzdata "zone.tab" assignments.
 */
const TIMEZONE_COUNTRY: Record<string, string> = {
  ...
  "America/El_Salvador": "SV", "America/Fortaleza": "BR",
  "America/Fort_Nelson": "CA", "America/Glace_Bay": "CA", "America/Godthab": "GL",
  ...
  "America/Nome": "US", "America/Noronha": "BR",
  ...
  "America/Port_of_Spain": "TT", "America/Porto_Velho": "BR",
  "America/Puerto_Rico": "PR", "America/Punta_Arenas": "CL",
  ...
  "Asia/Istanbul": "TR", "Asia/Jakarta": "ID", "Asia/Jayapura": "ID",
  ...
  "Asia/Rangoon": "MM", "Asia/Riyadh": "SA", "Asia/Sakhalin": "RU",
  ...
  "Asia/Thimphu": "BT", "Asia/Tokyo": "JP", "Asia/Tomsk": "RU",
  "Asia/Ulaanbaatar": "MN", "Asia/Urumqi": "CN", "Asia/Ust-Nera": "RU",
  "Asia/Vientiane": "LA", "Asia/Vladivostok": "RU", "Asia/Yakutsk": "RU",
  ...
  "Australia/Broken_Hill": "AU", "Australia/Darwin": "AU",
  "Australia/Eucla": "AU", "Australia/Hobart": "AU", "Australia/Lindeman": "AU",
  "Australia/Lord_Howe": "AU", "Australia/Melbourne": "AU",
  ...
  "Europe/Kirov": "RU", "Europe/Kyiv": "UA", "Europe/Lisbon": "PT",
  ...
  "Pacific/Pohnpei": "FM", "Pacific/Port_Moresby": "PG",
  "Pacific/Rarotonga": "CK", "Pacific/Saipan": "MP", "Pacific/Tahiti": "PF",
  "Pacific/Tarawa": "KI", "Pacific/Tongatapu": "TO", "Pacific/Truk": "FM",
  "Pacific/Wake": "UM", "Pacific/Wallis": "WF",
};

/** Derive a 2-letter ISO country code from an IANA timezone, if known. */
export function countryFromTimeZone(timezone?: string): string | undefined {
  if (!timezone) return undefined;
  return TIMEZONE_COUNTRY[timezone];
}

/** Derive a 2-letter ISO country code from a locale like "en-US" / "ro-RO". */
...
/** Derive a 2-letter language code from a locale like "en-US" → "en". */
export function languageFromLocale(locale?: string): string | undefined {
  if (!locale) return undefined;
  const match = locale.match(/^([A-Za-z]{2,3})/);
  return match?.[1]?.toLowerCase();
}

/**
 * Build a UserContext from the browser-provided timezone + locale headers.
 * Country is taken from the locale first (most reliable), then timezone.
 *
 * Country codes are normalized to lowercase (e.g. "us") because the search
 * APIs (Brave, NewsAPI) expect lowercase 2-letter codes.
 */
export function userContextFromHeaders(
  ...
  return {
    timezone: tz,
    country: (countryFromLocale(loc) ?? countryFromTimeZone(tz))?.toLowerCase(),
    language: languageFromLocale(loc),
  };
}
